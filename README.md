...
**페이지 진입 코드 + 앱별 개별 코드**로 보호되며, 다운로드 링크는 코드로 **AES 암호화**됩니다.
...
2. **사이트 설정**
   - 페이지 코드(진입 비번)를 정합니다. ⚠ **추측하기 어렵게 길게** 쓰세요.
3. **앱**별로
   - 이름·버전·플랫폼·설명 입력
   - **앱 코드**: 비우면 페이지 코드만으로 열림 / 입력하면 그 앱만 별도 코드 필요
...
- 다운로드 링크는 코드로 **실제 암호화**됩니다 → 코드 없이는 링크를 복호화 불가 (단순 숨김 아님).
- 정적 호스팅의 한계상:
  - 코드를 아는 사람이 **받은 파일은 재공유** 가능.
  - **약한 코드는 오프라인 추측(brute-force)** 위험 → 코드는 길고 무작위하게.
