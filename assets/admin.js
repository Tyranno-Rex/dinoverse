...
async function generate() {
  const pagePw = $('#cfg-pagepw').value;
  if (!pagePw) { setStatus('⚠ 페이지 코드를 입력하세요'); $('#cfg-pagepw').focus(); return; }
  ...
  const listItems = published.map((a) => ({
    id: a.id, name: a.name || a.id, description: a.description, version: a.version,
    platforms: a.platforms, lock: a.password ? 'own' : 'page',
  }));
  const data = {
    config: { title: $('#cfg-title').value.trim(), heroSub: $('#cfg-hero').value.trim() },
    list: await encryptJSON(listItems, pagePw),
    downloads: {},
  };
  for (const a of published) {
    const pw = a.password || pagePw;
    data.downloads[a.id] = await encryptJSON({ files: a.files, notes: '' }, pw);
  }
